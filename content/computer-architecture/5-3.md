...
2. TLB -> hit, Cache -> miss, VM -> hit
    - TLB가 hit이므로 page table 볼 필요가 없음
    - 그러나 Cache가 miss이므로 메모리 접근 1회 필요
3. TLB -> miss, Cache -> hit, VM -> hit
    - TLB가 miss이므로 Page table에 접근해서 가상주소 -> 실제주소 변환 필요 (메모리 접근 1회 필요)
    - Page table을 접근했는데 valid bit가 0이라면 page fault 발생
    - Cache는 hit이므로 더 이상 메모리 접근 없음
4. TLB -> miss, Cache -> miss, VM -> miss
    - Worst case
    - 최악의 경우로 memory에서 miss 발생 -> page fault
    - 즉 valid bit가 0이므로 OS가 제어를 넘겨받음
5. VM -> miss면, Cache와 TLB가 무조건 miss
